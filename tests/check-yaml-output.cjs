...
const wasmModulePath = path.join(__dirname, '..', 'wasm-lib', 'pkg', 'wasm_lib.js');

// Expected YAML output - should not contain srs: true
const expectedPatterns = [
    { pattern: /^reviews:\s*$/m, description: 'Should start with reviews field' },
    { pattern: /^  - /m, optional: true, description: 'May contain array items (optional)' },
];

const forbiddenPatterns = [
    { pattern: /srs:\s*true/, description: 'Should NOT contain srs: true field' },
    { pattern: /srs:\s*false/, description: 'Should NOT contain srs: false field' },
...
async function runTest() {
    ...
    if (!fs.existsSync(wasmModulePath)) {
        ...
        console.log('   ℹ️  Run "npm run build-wasm" first to build the module');
        return;
    }
            ...
            try {
                ...
                if (parsed && typeof parsed === 'object') {
                    ...
                    if (parsed.srs !== undefined) {
                        console.log('   ❌ Contains srs field (should be removed)');
                        allPassed = false;
                    } else {
                ...
                console.log(`   ⚠️  Cannot parse YAML (js-yaml not installed or error): ${error.message}`);
                console.log('   ℹ️  Install js-yaml for full validation: npm install js-yaml');
            }
        } else {
