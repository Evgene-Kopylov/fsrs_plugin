...
function parseModernFsrsFromFrontmatter(frontmatter, filePath) {
    try {
        ...
        if (!parsed) {
            return { success: false, card: null, error: 'Failed to parse YAML' };
        }

        if (parsed.srs !== true) {
            return { success: false, card: null, error: 'srs flag is not true' };
        }

        if (!parsed.reviews || !Array.isArray(parsed.reviews)) {
            return { success: false, card: null, error: 'reviews array is missing or invalid' };
        }

        const reviews = [];
        for (const session of parsed.reviews) {
            if (!session.date || !session.rating ||
    ...
    try {
        ...
        for (const file of mdFiles) {
            ...
            try {
                ...
                if (!frontmatter) {
                    console.log(`  ⚠️  No frontmatter found (might be expected)`);
                    skipped++;
                    continue;
                }

                const result = parseModernFsrsFromFrontmatter(frontmatter, filePath);
                ...
                if (result.success) {
                    ...
                    if (result.card.reviews.length > 0) {
                        const lastReview = result.card.reviews[result.card.reviews.length - 1];
                        console.log(`     Last review: ${lastReview.date} (${lastReview.rating})`);
                    }
                    passed++;
                } else {
