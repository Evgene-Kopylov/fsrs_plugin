...
/**
 * Форматирует интервал в днях для отображения
 * Показывает целые числа без десятичных знаков, дробные - с одним знаком
 */
function formatIntervalDays(days: number): string {
	...
	// Иначе показываем с одним десятичным знаком
 ...
 */
/**
 * Генерирует HTML таблицы для списка карточек на будущее
 */
export async function generateFsrsFutureHTML(
	cards: ModernFSRSCard[],
	settings: FSRSSettings,
	app: App,
	now: Date = new Date(),
	...
	let html = `<div class="fsrs-future-container">`;
	const formattedNow = formatDateTime(app, now);
	html += `<h4 class="fsrs-future-header"><span class="fsrs-header-text">Карточек на будущее: ${cards.length}</span><span class="fsrs-info-icon" title="Карточки, запланированные на будущие повторения. Отсортированы по дате следующего повторения.&#10;Интервал: запланированное количество дней до следующего повторения.&#10;&#10;Данные обновлены: ${formattedNow}">(?)</span></h4>`;

	html += `<table class="fsrs-future-table">`;
	html += `<thead><tr>`;
	html += `<th class="fsrs-col-file">Файл</th>`;
	html += `<th class="fsrs-col-reps">Повторений</th>`;
	html += `<th class="fsrs-col-interval">Интервал</th>`;
	html += `<th class="fsrs-col-next">Следующее повторение</th>`;
	html += `<th class="fsrs-col-in">Через</th>`;
	html += `</tr></thead><tbody>`;
	...
	for (const { card, state } of cardsWithState) {
		...
		if (diffDays === 0) {
			...
			inDaysText = `${diffHours} ч`;
		}

		// Форматируем интервал
		const intervalDays = state.scheduled_days || 0;
		const intervalText = formatIntervalDays(intervalDays);

		html += `<tr class="fsrs-future-row" data-file-path="${card.filePath}">`;
		html += `<td class="fsrs-col-file"><a href="${card.filePath}" data-file-path="${card.filePath}" class="internal-link">${displayName}</a></td>`;
		html += `<td class="fsrs-col-reps">${card.reviews.length}</td>`;
		html += `<td class="fsrs-col-interval">${intervalText}</td>`;
		html += `<td class="fsrs-col-next">${nextDateText}</td>`;
		html += `<td class="fsrs-col-in">${inDaysText}</td>`;
		html += `</tr>`;
	}
	...
	if (cards.length > maxCardsToShow) {
		const hiddenCount = cards.length - maxCardsToShow;
		html += `<div class="fsrs-future-info"><small>Показано: ${maxCardsToShow} из ${cards.length} карточек (${hiddenCount} скрыто)</small></div>`;
	}
