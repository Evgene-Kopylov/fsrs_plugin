...
- **\`SELECT *\`** — выбор всех доступных полей
...
- Можно использовать \`*\` для выбора всех доступных полей
...
- Если параметр ORDER BY не указан, используется сортировка по умолчанию ORDER BY overdue DESC
...
- Без указания параметра ORDER BY используется сортировка по умолчанию ORDER BY overdue DESC
