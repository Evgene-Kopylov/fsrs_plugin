...
mode: all
sort: due DESC
columns: file as "Файл", reps as "Повт.", due as "Следующее повторение", state as "Состояние"
\\\`\\\`\\\``;
...
## Параметры
- **\`mode: due|all\`** — режим отображения (обязательный)
- **\`limit: число\`** — ограничение количества строк (0 = используется значение из настроек плагина, обычно 30)
- **\`sort: поле ASC|DESC\`** — сортировка по указанному полю (ASC - по возрастанию, DESC - по убыванию)
...
- Если параметр sort не указан, используется сортировка по умолчанию для режима
...
- С параметром sort используется указанная пользователем сортировка
- В режиме due пользовательская сортировка применяется только к просроченным карточкам
- В режиме all пользовательская сортировка применяется ко всем карточкам
