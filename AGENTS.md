...
3. Отметить `[x]` в `index.md` и сделать коммит (код + отметка) → уведомить пользователя
4. Дождаться принятия коммита
5. Взять следующую
6. Если свободных нет — обновить `index.md` из `docs/tasks/`
...
- **Запрет хардкода цветов.** Никаких `#e53935`, `#999` в TS или CSS. Цвета — только через CSS-переменные (`var(--fsrs-color-again)`, `var(--text-faint)`) или настройки плагина.
- **Переиспользование существующего.** Прежде чем создать константу/функцию — проверить, нет ли уже нужного в `interfaces/fsrs.ts`, `constants.ts`, существующих модулях. Пример: `numberToRating()` уже есть, не нужен новый `RATING_KEYS`.
- **Запрет параллельных абстракций.** Если в коде уже есть `translateRating`/`colorOrDefault` — использовать их, а не писать `resolveRatingLabel`/`resolveRatingColor`. Один способ на одну операцию.
...
## Трейсинг пути данных при ревью

При ревью фичи, добавляющей новое поле/сущность, пройти всю цепочку сериализации:

1. Найти точку входа (YAML/JSON/пользовательский ввод)
2. Пройти парсинг → внутреннюю модель
3. Пройти все точки сериализации (JSON для TS, YAML для файла)
4. Найти все функции-дубликаты, которые могут сериализовать ту же структуру в обход каноничной
...
## Работа с GitLab (merge requests)

- «Сделай MR» / «Открой MR» / «Создай MR» → значит ветка уже запушена, push не нужен.
- Использовать инструмент `create_merge_request`:
  - `project_id` — `Evgene-Kopylov/FSRS-plugin`
