...
Универсальный блок `fsrs-table` заменяет собой отдельные блоки `fsrs-now` и `fsrs-future`, добавляя новый режим `all` для отображения всех карточек.
...
- **`due`** — карточки, готовые к повторению (просроченные) — аналог `fsrs-now`
- **`future`** — карточки на будущее (непросроченные) — аналог `fsrs-future`
...
- Фильтрует карточки в зависимости от выбранного режима
- Сортирует по приоритету (для режима `due` — по извлекаемости)
- Отображает выбранные колонки с информацией
...
**Функциональность:**
- Быстрое повторение карточки без перехода к блоку `fsrs-table`
