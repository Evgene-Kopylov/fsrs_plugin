...
|---------|----------------|
| `file` | имя заметки-карточки (кликабельная ссылка) |
| `R` | извлекаемость (Retrievability) — вероятность вспомнить сейчас |
| `S` | стабильность (Stability) — дни, за которые R упадёт до 90% |
| `D` | сложность (Difficulty) — от 0 до 10 |
| `Просрочка` | на сколько часов просрочено повторение |
