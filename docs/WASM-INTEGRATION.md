...
## Заметка о Mutex в WASM-кэше

Кэш карточек хранится как `static Mutex<HashMap<...>>`. WASM-модуль плагина выполняется в одном потоке — блокировки никогда не происходит. `Mutex` нужен только для удовлетворения требования Rust: `static`-переменная должна быть `Sync`, а `HashMap` не реализует `Sync`.

**Почему не `RefCell`:** `RefCell` не `Sync` — не подходит для `static`. Замена на `static mut` + `unsafe` неоправданна.

**Потенциал многопоточности:** FSRS-расчёты для каждой карточки независимы — их можно распараллелить на несколько потоков. Если Obsidian когда-нибудь позволит плагинам запускать Web Workers с доступом к WASM, `Mutex` станет реальной защитой общего кэша без изменений в коде.

### Направления исследования

Electron (на котором построен Obsidian) поддерживает Web Workers. Плагин может создать `new Worker()` из blob-URL и передать WASM-модуль через `postMessage`.

**Вариант A: Web Worker с вычислениями без кэша**

Worker получает stateless WASM-модуль без `static CACHE`. Основной поток передаёт пачки карточек (`{card_json, params_json}`), worker вычисляет FSRS-состояния и возвращает результаты. Основной поток обновляет свой кэш.

Плюс: не нужно синхронизировать состояние между потоками.
Минус: два WASM-бандла — с кэшем и без.

**Текущий статус:** исследование открыто. Приоритет — вариант A (worker без кэша) как наименее инвазивный.

#### UPD Резолюция:  
- **Многопоточность (Web Workers / wasm-bindgen-rayon) в текущих реалиях не имеет смысла.**  
- Основной бутылкой является обход 100k+ файлов через `getFileCache`, который не параллелится.  
- Вычислительная часть (FSRS) уже достаточно быстра.  
- `Mutex` в статическом кэше остаётся — он необходим для `Sync` и не создаёт блокировок в однопоточном окружении.
