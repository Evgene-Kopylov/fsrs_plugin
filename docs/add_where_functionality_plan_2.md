...
**Тесты:** Юнит-тесты evaluator’а с моками карточек, покрывающие все операторы и логические комбинации.
...
**7.5. Тестирование интеграции** – небольшой HTML/JS-скрипт, вызывающий WASM с WHERE-запросами.
...
- Полный цикл: парсинг блока → фильтрация карточек → проверка результатов
- Тест с реальными данными (несколько карточек с разными числовыми значениями)

**8.3. Бенчмарки (опционально)**
- Замер времени фильтрации 1000 карточек с WHERE-условием

**8.4. Документация**
- Обновить `README.md` (или внутреннюю документацию) – примеры WHERE-запросов
- Добавить комментарии в коде для новых публичных функций

---

## Хронология выполнения (оценочно)

| Шаг | Описание | Время (часы) |
|-----|----------|---------------|
| 1   | Лексер | 1 |
| 2   | AST (expression.rs) | 0.5 |
| 3   | Парсер (where clause + expression) | 3 |
| 4   | Валидатор | 1 |
| 5   | Evaluator | 2 |
| 6   | Интеграция с filter_and_sort_cards | 1 |
| 7   | WASM + TypeScript | 2 |
| 8   | Тесты + документация | 3 |
| **Итого** | | **13.5** |
