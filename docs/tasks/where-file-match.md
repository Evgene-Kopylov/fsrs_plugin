...
- [x] Интеграционные тесты в `cache.rs` (полный SQL-парсинг + query_cards): `test_query_cards_with_where_file_stem`, `test_query_cards_with_where_file_spaces_parens`
- [x] 176 тестов проходят
- [x] Консоль-лог в Obsidian подтверждает стемы
- [ ] Пользователь сообщает «ничего не работает» — WHERE по-прежнему не находит карточки

## Гипотезы для проверки

1. **Файла с именем из WHERE действительно нет в кэше.** В логах `'STEND'` нет среди стемов. Нужно проверить, какой именно файл ожидает пользователь и есть ли он в хранилище.

2. **Юникод-нормализация.** `Path::file_stem()` и значение из парсера WHERE могут использовать разные формы нормализации (NFC/NFD) — сравнение `==` байтовое, не сработает. Пример: macOS NFD vs Linux/WASM NFC.

3. **Регистр букв.** Сравнение точное, без `to_lowercase()`. `"Stend" != "STEND"`.

4. **Проблема не в `file`, а в другой части WHERE.** Если `WHERE file = "имя" AND retrievability > 0.5` не работает из-за `retrievability`, а не `file`.
