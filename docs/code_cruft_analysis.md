...
### Этап 1: Удаление явно неиспользуемого кода (Низкий риск) ✅ ВЫПОЛНЕНО
...
1. **Удалить функции-заглушки** в `fsrs-table-filter.ts`:
   ...
   ✅ **ВЫПОЛНЕНО**: Удалены функции `getFieldValue`, `sortCardsForDue`, `sortScheduledCards`, `calculatePriorityScore`, `computeCardsStates`, `applyDefaultSort`, `applyCustomSort`

2. **Удалить `review-current-card.ts`**:
   - Проверить, что нет импортов из этого файла
   - Обновить документацию, если нужно
   ✅ **ВЫПОЛНЕНО**: Файл удален, импорты проверены (используется только через команду плагина)
...
3. **Удалить `filterAndSortCardsWithSql`**:
   ...
   ✅ **ВЫПОЛНЕНО**: Функция удалена, обновлена `generateTableHTMLFromSql` для использования `parseSqlBlock() + filterAndSortCards()`

**Дополнительно выполнено:**
- Удален интерфейс `LegacyFSRSCard` из `src/interfaces/fsrs.ts`
- Удалена устаревшая функция `reviewCurrentCardSimple` из `review-card.ts`
- Удалена устаревшая функция `parseTableParams` из `fsrs-table-params.ts`
- Убран отладочный вывод `console.info` из `parseSqlBlock`


### Этап 2: Консолидация дублирующей функциональности (Средний риск) ✅ ЧАСТИЧНО ВЫПОЛНЕНО
...
1. **Унифицировать функции фильтрации**:
   ...
   ✅ **ВЫПОЛНЕНО**: Удалена дублирующая функция `filterAndSortCardsWithSql`, оставлена только `filterAndSortCards`. Обновлена `generateTableHTMLFromSql` для использования `parseSqlBlock() + filterAndSortCards()`

2. **Упростить цепочку реэкспортов**:
   - Сократить глубину реэкспортов с 3+ уровней до 1-2
   - Явно экспортировать только публичное API
   ⚠️ **ЧАСТИЧНО ВЫПОЛНЕНО**: Удален реэкспорт `filterAndSortCardsWithSql` из `fsrs-table-helpers.ts`. Глубина реэкспортов (3+ уровня) остается для дальнейшего рефакторинга
...
3. **Стандартизировать сигнатуры функций**:
   ...
   ❌ **НЕ ВЫПОЛНЕНО**: Требуется дальнейшая работа по унификации сигнатур функций в разных модулях


### Этап 3: Улучшение типизации и архитектуры (Высокий риск) ❌ НЕ ВЫПОЛНЕНО
...
1. **Заменить "as any" на правильные типы**:
   ...
   ❌ **НЕ ВЫПОЛНЕНО**: Множественные "as any" и "as unknown" остаются в критических местах (например, в `fsrs-table-filter.ts`, `date-format.ts`)
...
2. **Разделить большие модули**:
   ...
   ❌ **НЕ ВЫПОЛНЕНО**: `fsrs-table-filter.ts` остается большим модулем (489 строк), нарушающим принцип единой ответственности
...
3. **Внедрить четкие слои**:
   ...
   ❌ **НЕ ВЫПОЛНЕНО**: Смешение WASM и TypeScript логики, отсутствие четкого разделения слоев


### Этап 4: Очистка и документация (Низкий риск) ✅ ЧАСТИЧНО ВЫПОЛНЕНО

1. **Убрать отладочный вывод**:
   - Удалить `console.info` и `console.warn` из продакшен кода
   - Заменить на структурированное логирование
   ✅ **ЧАСТИЧНО ВЫПОЛНЕНО**: Удалены `console.info` из `parseSqlBlock`, но остаются `console.warn` в различных модулях

2. **Обновить документацию**:
   - Четко указать, какие методы устарели
   - Предоставить migration path
   ⚠️ **ЧАСТИЧНО ВЫПОЛНЕНО**: Этот файл аналитики обновлен, но необходима полная документация по публичному API

3. **Добавить тесты**:
   - Покрыть критическую функциональность
   - Тестировать обработку ошибок
   ❌ **НЕ ВЫПОЛНЕНО**: Тесты для критической функциональности не добавлены

## Приоритеты исправлений

| Приоритет | Что исправить | Сложность | Риск | Статус |
|-----------|---------------|-----------|------|--------|
| Высокий | Удалить функции-заглушки | Низкая | Низкий | ✅ ВЫПОЛНЕНО |
| Высокий | Удалить неиспользуемый код | Низкая | Низкий | ✅ ВЫПОЛНЕНО |
| Средний | Консолидировать дубликаты | Средняя | Средний | ✅ ЧАСТИЧНО ВЫПОЛНЕНО |
| Средний | Убрать отладочный вывод | Низкая | Низкий | ✅ ЧАСТИЧНО ВЫПОЛНЕНО |
| Низкий | Улучшить типизацию | Высокая | Высокий | ❌ НЕ ВЫПОЛНЕНО |
| Низкий | Рефакторинг архитектуры | Высокая | Высокий | ❌ НЕ ВЫПОЛНЕНО |

## Статус выполнения

На основе анализа были выполнены исправления кривизн в коде FSRS плагина:

### ✅ Выполнено полностью (Этап 1)
- Удалены 7 функций-заглушек из `fsrs-table-filter.ts`
- Удален файл `review-current-card.ts` 
- Удалена функция `filterAndSortCardsWithSql`
- Удален интерфейс `LegacyFSRSCard`
- Удалена функция `reviewCurrentCardSimple`
- Удалена функция `parseTableParams`

### ⚠️ Частично выполнено (Этап 2 и 4)
- Консолидированы дублирующие функции фильтрации
- Убран отладочный вывод `console.info` из критических мест
- Обновлен этот файл аналитики

### ❌ Не выполнено (Этап 3)
- Улучшение типизации (замена "as any")
- Рефакторинг архитектуры (разделение модулей)
- Внедрение четких слоев
