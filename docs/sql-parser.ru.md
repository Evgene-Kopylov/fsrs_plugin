...
**Неизвестное поле в WHERE:** валидатор выдаёт предупреждение, и при выполнении запроса ошибка `UnknownField` попадает в `result.errors`. Таблица показывает сообщение об ошибке с указанием неверного поля.
