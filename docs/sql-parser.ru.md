...
После парсинга валидатор проверяет собранную структуру и выдаёт предупреждения (не ошибки — таблица всё равно рендерится):

| Проверка | Нарушение | Предупреждение |
|---|---|---|
| Поле в SELECT существует? | `SELECT unknown_field` | `UnknownField("unknown_field")` |
| Поле в WHERE существует? | `WHERE foo = 1` | `UnknownField("foo")` |
| Поле в ORDER BY существует? | `ORDER BY bar` | `UnknownField("bar")` |
| Нет дубликатов в SELECT? | `SELECT file, file` | `DuplicateField("file")` |
| Лимит не слишком большой? | `LIMIT 99999` | `LargeLimit(99999)` |

**Неизвестное поле в WHERE — особая ситуация:** валидатор выдаёт предупреждение, но таблица всё равно пытается выполниться. При вычислении условия каждая карточка получает ошибку `UnknownField` и отфильтровывается. Результат: таблица пустая.
